classdef IncSemiSVM
    ...
    methods(Static = true)
        ...
        function out=Run(original_y,extended_x,extended_y,added_x,added_y,initial_solution)
            global add_size
            nonLabel_index=ones(1,add_size);
            ...
            extended_x=[extended_x;tmp_x];
            extended_y=[extended_y;tmp_y];
            svm_struct.alpha = [svm_struct.alpha;zeros(length(tmp_y),1)];
            ...
            [svm_struct,index_A,direction_A]=IncSemiSVM.InitialA(svm_struct,g(end-add_size+1:end,1),length(extended_y),added_y);
...
%             other_set(end)=[];  %% delete the candidate sample from the other set
        ...
        function [svm_struct,index_A,direction_A] = InitialA(svm_struct,gc,index_added,added_y)
            global add_size fake_zero
            index_A=[];
            direction_A=[];
            alpha_c=svm_struct.alpha(end-add_size+1:end,1);
            pms=svm_struct.pms(end-add_size+1:end,:);
